  ...
  return (
    <View style={styles.hero}>
      <View style={styles.heroCopy}>
        ...
        <Text style={styles.heroTitle}>{focusChapter ? 'Continue your learning path.' : 'Build your learning path.'}</Text>
        <Text style={styles.heroBody}>{focusChapter ? 'Start with the chapter that can improve your next result most.' : 'Complete a paper and Eduraa will shape your first evidence-based route.'}</Text>
      </View>
      <View style={styles.routeStage}>
        ...
        <View style={styles.routeLabel}>
          ...
          <Text style={styles.routeMeta}>{mastery == null ? 'Waiting for your first learning signal' : `${Math.round(mastery)}% mastery · ${focusChapter?.topics_count ?? 0} topics`}</Text>
        </View>
      </View>
    </View>
  )
...
function MetricSummary({
  ...
  const items = [
    { label: 'Created', value: `${generated}`, helper: 'practice papers', tone: colors.accent },
    { label: 'Attempted', value: `${attempts}`, helper: 'submissions', tone: colors.info },
    { label: 'Checked', value: `${checked}`, helper: 'results ready', tone: colors.success },
  ]
  ...
  return (
    <View style={styles.planCard}>
      ...
      <View style={styles.planCopy}>
        ...
        <Text style={styles.planBody} numberOfLines={2}>{focusChapter ? `${chapterLabel} is the clearest repair signal from your recent work.` : 'Complete your first paper so Eduraa can identify the most useful concept to repair.'}</Text>
        <View style={styles.planPills}>
          <Text style={styles.planPill}>{mastery == null ? 'No mastery signal yet' : `${Math.round(mastery)}% mastery`}</Text>
          {topics != null ? <Text style={styles.planPill}>{topics} {topics === 1 ? 'topic' : 'topics'}</Text> : null}
        </View>
...
function CompetitiveHome({
  ...
  const trackLabel = user?.b2c_board || user?.b2c_target_exam || 'JEE Mains / Advanced'
  ...
  const subjectData = useMemo(() => {
    ...
    return subjects.map((name) => {
      const chapters = (analytics?.chapter_mastery ?? []).filter(
        (ch) => (ch.subject ?? '').toLowerCase().includes(name.toLowerCase()),
      )
      const avg = chapters.length
        ? Math.round(chapters.reduce((s, c) => s + (c.mastery ?? 0), 0) / chapters.length)
        : 0
      return { name, avg, count: chapters.length, tone: subjectTone(name), symbol: subjectSymbol(name), gradient: subjectColors[name] }
    })
  ...
  const overallReadiness = useMemo(() => {
    ...
    return Math.round(withData.reduce((s, d) => s + d.avg, 0) / withData.length)
  }, [subjectData])
  ...
  return (
    <AppScreen
      tone="auth"
      ambient={false}
      refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor="#f36c21" colors={['#f36c21']} />}
    ...
    >
      ...
      <View style={styles.compHeader}>
        ...
        <View style={styles.headerCopy}>
          <Text style={styles.compHeaderGreeting}>{greeting}, {model.firstName}</Text>
          <Text style={styles.headerDetail} numberOfLines={1}>{trackLabel}</Text>
        </View>
        <Pressable onPress={() => navigation.navigate('Profile')} style={styles.compHeaderBell}>
        ...
        >
          <View style={styles.compHeroTop}>
            ...
            <View style={styles.compHeroStat}>
              <Text style={styles.compHeroStatValue}>{overallReadiness != null ? `${overallReadiness}%` : '--'}</Text>
              <Text style={styles.compHeroStatLabel}>readiness</Text>
            </View>
          </View>

          <Text style={styles.compHeroTitle}>
            {focusChapter ? `Let's master\n${focusChapter.chapter}` : 'Start your JEE\nprep journey'}
          </Text>
          ...
          <View style={styles.compGlassCard}>
            ...
            <Text style={styles.compGlassTitle}>
              {focusChapter ? focusChapter.chapter : 'Complete your first paper'}
            </Text>
            ...
            {focusChapter?.mastery != null ? (
              <View style={styles.compGlassProgress}>
                <View style={[styles.compGlassProgressFill, { width: `${Math.max(8, focusChapter.mastery)}%` }]} />
              </View>
            ) : null}
            <Pressable
              onPress={() => navigation.navigate('Papers', { screen: 'GeneratePaper' })}
              style={({ pressed }) => [styles.compHeroCta, pressed && styles.pressed]}
            >
      ...
      {focusChapter ? (
        <View style={styles.compAiCard}>
          ...
          <Text style={styles.compAiText}>
            I noticed your weakest area is <Text style={styles.compAiHighlight}>{focusChapter.chapter}</Text> at {focusChapter.mastery != null ? `${Math.round(focusChapter.mastery)}%` : 'low'} mastery. A focused session here could significantly boost your JEE readiness score.
          </Text>
          <View style={styles.compAiActions}>
            <Pressable
              onPress={() => navigation.navigate('Learning', { screen: 'AgenticLearning' })}
              style={({ pressed }) => [styles.compAiPrimary, pressed && styles.pressed]}
            >
              <Text style={styles.compAiPrimaryText}>Start learning</Text>
            </Pressable>
            <Pressable
              onPress={() => navigation.navigate('Learning', { screen: 'AIStudio' })}
              style={({ pressed }) => [styles.compAiSecondary, pressed && styles.pressed]}
            >
      ...
      <View style={styles.compSectionHeader}>
        ...
        <Text style={styles.compSectionMeta}>{overallReadiness != null ? `${overallReadiness}% overall` : 'No data yet'}</Text>
      </View>
      ...
      <View style={styles.compSubjectRow}>
        {subjectData.map((subject) => (
          <Pressable
            ...
            style={({ pressed }) => [styles.compSubjectCard, pressed && styles.pressed]}
          >
            <LinearGradient
              colors={subject.gradient ? [subject.gradient.start, subject.gradient.end] : [subject.tone, subject.tone]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
            ...
            <View style={styles.compSubjectBar}>
              <LinearGradient
                ...
                style={[styles.compSubjectFill, { width: `${Math.max(6, subject.avg)}%` }]}
              />
            </View>
            <Text style={styles.compSubjectMeta}>
              {subject.count > 0 ? `${subject.avg}% · ${subject.count} ch` : 'Not started'}
            </Text>
          </Pressable>
        ))}
      </View>
      ...
      <Pressable
        onPress={() => navigation.navigate('Learning', { screen: 'CompetitiveExam' })}
        style={({ pressed }) => [styles.compLaunchpad, pressed && styles.pressed]}
      >
        <LinearGradient
        ...
        >
          <View style={styles.compLaunchpadLeft}>
            ...
            <View style={styles.compLaunchpadCopy}>
              ...
              <Text style={styles.compLaunchpadBody}>Chapter workspaces, cheat sheets & revision packs</Text>
            </View>
          </View>
      ...
      <View style={styles.compSectionHeader}>
        ...
        <Text style={styles.compSectionMeta}>{recentSubmissions.length > 0 ? `${recentSubmissions.length} recent` : 'No activity yet'}</Text>
      </View>
...
function QuickAction({
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.compQuickCard, pressed && styles.pressed]}>
      <View style={[styles.compQuickIcon, { backgroundColor: `${tone}12` }]}>
        <Ionicons name={icon} size={18} color={tone} />
      </View>
      <Text style={styles.compQuickLabel}>{label}</Text>
      <Text style={styles.compQuickBody} numberOfLines={2}>{body}</Text>
    </Pressable>
  )
}
...
export default function HomeScreen({ competitive = false }: { competitive?: boolean }) {
  const navigation = useNavigation<any>()
  const { user } = useAuthStore()
      ...
      {isLoading ? (
        ...
        <>
          <Hero
            ...
            onAskAi={() => navigation.navigate('Learning', { screen: 'AIStudio' })}
          />

          <MetricSummary generated={model.generatedPapers} attempts={model.attempts} checked={model.checked} />

          <SectionRow title="Next best action" meta={model.focusChapter ? 'Based on recent work' : 'Start here'} />
