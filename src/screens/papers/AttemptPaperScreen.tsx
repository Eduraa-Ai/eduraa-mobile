...
export default function AttemptPaperScreen() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const gradingStatus = String(
        (data as { grading_status?: string }).grading_status || "",
      ).toLowerCase();
      const isChecking =
        gradingStatus === "submitted" || gradingStatus === "checking";
      const scoreText =
      ...
      if (isExistingSubmission) {
        setSubmitOutcome({
          ...
          message:
            "This paper already has a recorded submission. Each paper can only be attempted once.",
        ...
        try {
          ...
          if (existing?.id) {
            setSubmitOutcome({
              ...
              scoreText:
                existing.total_score != null && existing.max_score
  ...
  const holdSecondsLeft = Math.max(
    0,
    Math.ceil(((100 - submitHoldProgress) / 100) * (HOLD_DURATION_MS / 1000)),
  );
          ...
          {timeLeft !== null ? (
            <View style={[styles.timer, timeLeft < 300 && styles.timerWarning]}>
              <Ionicons
                name="time-outline"
                size={14}
                color={timeLeft < 300 ? colors.white : colors.accent}
              />
          ...
          >
            <View style={styles.questionHeader}>
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.flagButton,
                  flagged[q.id] && styles.flagButtonActive,
                ]}
              >
                <Ionicons
              ...
              <View style={styles.mcqOptions}>
                {q.options.map((opt, i) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.mcqOption,
                      answers[q.id] === opt.id && styles.mcqOptionSelected,
                    ]}
                    onPress={() =>
              ...
              <View style={styles.tfRow}>
                {["True", "False"].map((val) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.tfBtn,
                      answers[q.id] === val && styles.tfBtnSelected,
                    ]}
                    onPress={() =>
          ...
          >
            ...
            {submitOutcome.scoreText ? (
              <View style={styles.submittedScoreBox}>
                ...
                <Text style={styles.submittedScoreValue}>
                  {submitOutcome.scoreText}
                </Text>
              </View>
            ) : submitOutcome.kind !== "error" ? (
            ...
            <View style={styles.submitActions}>
              {submitOutcome.kind === "error" ? (
                ...
                <>
                  <TouchableOpacity
                    activeOpacity={0.85}
                    onPress={() => navigation.navigate("PapersList")}
                    style={styles.submitCancelButton}
                  >
