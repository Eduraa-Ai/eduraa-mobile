...
const fullScreenNestedRoutes = new Set([
  "AttemptPaper",
  "Quiz",
  "AIStudio",
  "StaffAIStudio",
...
function getNestedFocusedRouteName(
  route: BottomTabBarProps["state"]["routes"][number],
) {
  const helperName = getFocusedRouteNameFromRoute(route);
  if (helperName) return helperName;

  let nestedState = route.state as
    | { index?: number; routes?: Array<{ name?: string; state?: unknown }> }
    | undefined;
...
export function BottomTabBar({
  ...
  const focusedOptions = focusedRoute
    ...
    : undefined;
  const nestedRouteName = focusedRoute
    ? getNestedFocusedRouteName(focusedRoute)
    : null;
  ...
  const contentWidth = Math.max(
    ITEM_SIZE,
    state.routes.length * ITEM_SIZE +
      Math.max(0, state.routes.length - 1) * ITEM_GAP +
      1,
  );
  ...
  const scrollToIndex = useCallback(
    (index: number, animated = true) => {
      ...
      const centeredScroll = clamp(
        thumbPosition - (viewportWidth - ITEM_SIZE) / 2,
        0,
        maxScroll,
      );
      ...
      PanResponder.create({
        ...
        onPanResponderMove: (_, gesture) => {
          const maxPosition = Math.max(
            0,
            (state.routes.length - 1) * SLOT_SIZE,
          );
          const nextPosition = clamp(
            state.index * SLOT_SIZE + gesture.dx,
            0,
            maxPosition,
          );
          const nextPreviewIndex = clamp(
            Math.round(nextPosition / SLOT_SIZE),
            0,
        ...
        onPanResponderRelease: (_, gesture) => {
          const nearestIndex = clamp(
            Math.round(dragPositionRef.current / SLOT_SIZE),
            0,
            state.routes.length - 1,
          );
    ...
    <View style={styles.wrap}>
      <View style={styles.barShell}>
        ...
        >
          ...
          >
            <Ionicons
              name={iconByRoute[state.routes[previewIndex]?.name] ?? "ellipse"}
              size={20}
              color={colors.white}
            />
            ...
            return (
              ...
              >
                <Ionicons
                  ...
                  size={20}
                  color={isFocused ? "transparent" : colors.textSecondary}
                />
              </Pressable>
            );
          })}
