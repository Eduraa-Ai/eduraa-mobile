...
When the user says `StartMobile`, asks to use the StartMobile skill, or requests the app on localhost and Expo Go with a fresh QR, use the repository-local `start-mobile` skill at `.codex/skills/start-mobile/SKILL.md`.
...
After every meaningful mobile UI implementation, use the repository-local `eduraa-premium-ui-critic` skill at `.codex/skills/eduraa-premium-ui-critic/SKILL.md` in an independent fresh-agent review. Give the critic rendered screenshots and task context, but do not disclose the builder's intended verdict or defend the design. A UI task is not visually complete until the critic returns `PASS`. On `REJECT`, implement the critic's highest-leverage feedback, recapture the real device states, and repeat. Do not substitute self-review, source inspection, compilation, or user praise for this gate.
