...
test('attendance API carries optimistic concurrency and correction contracts', () => {
  ...
  assert.match(api, /getStudentHistory/)
  assert.match(api, /attendance\/students\/me\/history/)
  assert.match(api, /createLeaveApplication/)
...
test('teacher leave decisions use an in-app confirmation that works on web', () => {
  ...
  assert.match(screen, /function LeaveDecisionDialog/)
  assert.match(screen, /<Modal/)
  assert.match(screen, /onResolve=\{requestLeaveDecision\}/)
  assert.match(screen, /confirmReady=\{leaveDecisionReady\}/)
  assert.match(screen, /disabled=\{busy \|\| !confirmReady\}/)
  assert.match(screen, /resolveLeaveMutation\.mutate\(/)
  assert.doesNotMatch(screen, /onResolve=\{\(item, status, note\) => Alert\.alert\(/)
})
